...
namespace Maispace\MaiTeam\Indexer;
...
class TeamMemberIndexer extends AbstractIndexer implements SearchResultFormatterInterface
{
    private const TABLE_NAME = 'tx_maiteam_team_member';
    ...
    protected function buildContent(object $record): string
    {
        ...
        $parts = [$record->getFullName()];

        if ($record->getRole() !== '') {
            $parts[] = $record->getRole();
        }

        if ($record->getBio() !== '') {
            $parts[] = strip_tags($record->getBio());
        }
    ...
    protected function getRecordsForIndexing(IndexingContext $context): iterable
    {
        ...
        return $dataMapper->map(TeamMember::class, $rows);
    }
